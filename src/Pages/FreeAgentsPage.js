...
const FreeAgentsPage = () => {
  ...
  const getFA = async () => {
    ...
    setFilteredFa(JSON.parse(dataNBA.body).concat(JSON.parse(dataNHL.body)));
  };
  ...
  const addPlayer = async (playerId) => {
    ...
    const teams = Object.values(await teamAccessor.getLeaugesTeams(leagueName));
    ...
    for (let key in userTeamCopy.CurrentLineup) {
      if (key.startsWith("BCH") && userTeamCopy.CurrentLineup[key] === null) {
