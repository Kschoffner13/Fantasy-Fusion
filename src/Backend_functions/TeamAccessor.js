...
class TeamAccessor {
    ...
    async getTeams(){
        const posts = await DataStore.query(Team,(c) => c.fantasyleagueID.eq(this.fantasyleagueID) );
        return posts;
    }
      ...
      const response = await DataStore.save(
        FantasyLeague.copyOf(original[0], (updated) => {
          for (const key in dic) {
            if (dic[key] != updated[key] && key !== "fantasyleagueID" && key !== "id") {
              updated[key] = dic[key];
            }
          }
        })
    ...
    async getTeamAttribute(ID, attribute) {
      const response = await DataStore.query(Team, (c) =>
        c.and((c) => [c.id.eq(ID), c.fantasyleagueID.eq(this.fantasyleagueID), c.UserID.eq(this.userID)])
      );
